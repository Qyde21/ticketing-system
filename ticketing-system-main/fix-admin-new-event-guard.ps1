...
# Usage: powershell -ExecutionPolicy Bypass -File fix-admin-new-event-guard.ps1
#
# Adds a server-side admin auth guard to /admin/events/new, matching the
# pattern used by every other admin page. The underlying API (/api/events)
# was already properly secured — this closes the cosmetic gap of the page
# itself being viewable (though not usable) by non-admins.
...
Write-Host "Writing: app\admin\events\new\NewEventForm.tsx" -ForegroundColor Cyan
$content = @'
'use client';
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-xl mx-auto py-12 px-4 text-white space-y-6">
      ...
      <label className="flex items-center gap-2 text-sm text-gray-300 bg-gray-900 border border-gray-800 rounded-xl p-4 cursor-pointer">
        ...
        Publish immediately (otherwise this event is saved as a draft and won&apos;t be visible to buyers until published)
      </label>
...
'@
Write-ClaudeFile "app\admin\events\new\NewEventForm.tsx" $content
if (-not (Test-Path -LiteralPath "app\admin\events\new\NewEventForm.tsx")) {
...
if ($script:anyFailed) {
    ...
    Write-Host "  git add ."
    Write-Host "  git commit -m ""Add server-side admin guard to /admin/events/new"""
