      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8 pb-4 border-b border-gray-800">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            ...
            {eventEnded && <span className="text-amber-400"> · Event ended — refunds disabled</span>}
          </p>
        </div>
          ...
          {orders.map((order: any) => (
            ...
            >
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <span className={`px-2.5 py-0.5 rounded-md text-xs font-bold uppercase tracking-wider ${
                    order.payment_status === 'completed' || order.payment_status === 'paid' || order.payment_status === 'success'
                ...
                <p className="text-xs text-gray-400 mt-1">
                  ...
                  <span className="text-cyan-300 font-semibold">{order.ticket_name || 'Standard Ticket'}</span> (Qty: {order.quantity})
                </p>
              ...
              <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end pt-4 md:pt-0 border-t md:border-t-0 border-gray-800">
                ...
                {order.payment_status !== 'refunded' && eventEnded && (
                  <span className="text-xs text-gray-500">Refund closed</span>
                )}
              </div>
            </div>
          ))}
