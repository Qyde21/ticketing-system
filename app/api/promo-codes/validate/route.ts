...
export async function POST(req: NextRequest) {
  try {
    ...
    const { eventId, ticketTypeId, quantity, code, subtotalKes: clientSubtotal } = body;

    if (!eventId || !code) {
      return NextResponse.json({ valid: false, error: 'Missing required fields' }, { status: 400 });
    }

    let subtotalKes: number;

    if (clientSubtotal !== undefined && clientSubtotal !== null && Number.isFinite(Number(clientSubtotal))) {
      subtotalKes = Math.max(0, Number(clientSubtotal));
      ...
      subtotalKes = Number(ticketType.price_kes) * qty;
    } else {
      return NextResponse.json({ valid: false, error: 'Missing required fields' }, { status: 400 });
    }
