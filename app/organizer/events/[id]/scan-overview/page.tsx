...
export default async function OrganizerScanOverviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (session.role === 'admin') {
      const results = await sql`SELECT id, title, venue_name, start_at, organizer_id FROM events WHERE id = ${eventId}`;
      event = results[0] || null;
    } else {
      const results = await sql`SELECT id, title, venue_name, start_at, organizer_id FROM events WHERE id = ${eventId} AND organizer_id = ${session.userId}`;
      event = results[0] || null;
    }
  ...
  return (
    <div className="max-w-2xl mx-auto py-10 px-4 text-white">
      ...
      <p className="text-gray-400 text-sm">{event.venue_name || 'No Venue'} &middot; {event.start_at ? new Date(event.start_at).toLocaleString() : 'No Date'}</p>
