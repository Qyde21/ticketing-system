...
export default function StaffManager({ eventId, initialStaff }: { eventId: string; initialStaff: StaffMember[] }) {
  ...
  async function handleRemove(userId: string) {
    setError('');
    setMessage('');
    const res = await fetch(`/api/events/${eventId}/staff?userId=${encodeURIComponent(userId)}`, { method: 'DELETE' });
    if (res.ok) {
    ...
    <div className="space-y-6">
      <form onSubmit={handleInvite} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-3">
        ...
        <p className="text-xs text-gray-500">
          They must already have a TicketHub account. Staff can scan tickets for this event only.
        </p>
        <div className="flex flex-col sm:flex-row gap-2">
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="staff@example.com"
            className="flex-1 bg-gray-950 border border-gray-800 rounded-xl px-3 py-2.5 text-sm text-white" />
      ...
      <div>
        <h2 className="text-sm font-bold uppercase tracking-wider text-gray-400 mb-3">Current staff ({staff.length})</h2>
        {staff.length === 0 ? (
          <p className="text-gray-500 text-sm">No staff yet.</p>
        ) : (
          <ul className="space-y-2">
            {staff.map((s) => (
              <li key={s.id} className="flex items-center justify-between gap-3 bg-gray-900 border border-gray-800 rounded-xl px-4 py-3">
                <div>
                  <p className="font-semibold text-white text-sm">{s.full_name}</p>
                  <p className="text-xs text-gray-400">{s.email}</p>
                </div>
                <button type="button" onClick={() => void handleRemove(s.id)}
